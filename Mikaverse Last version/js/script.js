...
function initStoreFilters() {
  const buttons = document.querySelectorAll(
    ".store-catalog .filter-btn, .store-toolbar .filter-btn",
  );
...
function initCompaniesProjectFilters() {
  const group = document.querySelector("[data-companies-filters]");
  const cards = document.querySelectorAll("[data-companies-cards] .company-project-card");
  ...
  buttons.forEach((button) => {
    button.addEventListener("click", () => {
      ...
      cards.forEach((card) => {
        const isVisible = card.dataset.filter === filter;
        card.style.display = isVisible ? "" : "none";
      });
    });
  });
...
function initProjectDetailToc() {
  const layout = document.querySelector("[data-project-detail]");
  if (!layout) return;
...
function getRootPrefix() {
  const depth = getPathDepthFromHtml();
...
function prefixNavHref(href, htmlPrefix) {
...
function loadNavbar() {
  ...
  fetch(`${getRootPrefix()}components/navbar.html`)
    .then((res) => {
    ...
    .then((html) => {
      ...
      placeholder.querySelectorAll("a[href]").forEach((link) => {
        const href = link.getAttribute("href");
        if (href) link.setAttribute("href", prefixNavHref(href, htmlPrefix));
      });
      initNavInteractions();
      initStoreFilters();
      initCompaniesProjectFilters();
